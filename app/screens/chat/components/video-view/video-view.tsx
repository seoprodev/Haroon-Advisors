...
export function VideoView(props: VideoViewInterface) {
  ...
  useEffect(() => {
    const updateVideoStatus = () => {
      if (!user) return;
      (async () => {
        isMounted() && user && user.videoStatus && setIsVideoOn(await user.videoStatus.isOn());
      })();
    };
  ...
  const resolution = videoResolution || VideoResolution.ResolutionAuto;

  return (
    <TouchableWithoutFeedback
      onPress={() => onPress(user)}>
      <View style={containerStyle}>
        {(isVideoOn || sharing || preview) && !isSharingCamera ? (
          <ZoomView
          ...
          < CameraView style={styles.zoomView} />
        ) : (
